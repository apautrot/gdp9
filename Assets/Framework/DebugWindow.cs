...
public class DebugWindow : SceneSingleton<DebugWindow>
{
	...
	private void OnGUI ()
	{
		...
		if ( !isVisible )
			GUI.Label ( new Rect ( 10, 5, 800, 25 ), "press F1 to debug", style );
		else
		{
			float y = 5;

			if ( values.Count == 0 )
				GUI.Label ( new Rect ( 10, 5, 800, 25 ), "empty debug window", style );
			else
			{
				...
				{
					{
						...
						GUI.Label ( new Rect ( 10, y, size.x, size.y ), text, style );
						y += size.y;
					}
					...
					foreach ( KeyValuePair<string, string> kvp in entry.Value )
					{
...

						{
							...
							GUI.Label ( new Rect ( x, y, 800, height ), text, style );
							y += height;
						}
					}
